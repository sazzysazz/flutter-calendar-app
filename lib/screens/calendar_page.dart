...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Map<DateTime, List<Holiday>> _holidayEvents = {};
  ...
  Future<void> loadHolidays() async {
    ...
    final Map<DateTime, List<Holiday>> mapped = {};
    for (var event in allEvents) {
      final day = event.date.stripTime();
      mapped.putIfAbsent(day, () => []).add(event);
    }

    if (mounted) {
      setState(() {
        _allEvents = allEvents;
        _holidayEvents = mapped;
      });
  ...
  List<Holiday> getEvents(DateTime day) {
    return _holidayEvents[day.stripTime()] ?? [];
  }
          ...
          SliverToBoxAdapter(
            child: Container(
              constraints: BoxConstraints(
                maxHeight: MediaQuery.of(context).size.height * 0.5, // limit to ~50% of screen
              ),
        ...
        SliverToBoxAdapter(
          child: Padding(
            padding: EdgeInsets.symmetric(
              horizontal: 16,
              vertical: _searchResults.isNotEmpty ? 0 : 8, // less top padding when results are shown
            ),
            child: Container(
              decoration: BoxDecoration(
                color: surfaceColor,
          ...
          SliverList(
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                ...
                return Padding(
                  padding: EdgeInsets.fromLTRB(24, index == 0 ? 0 : 8, 24, index == getEvents(_selectedDay).length - 1 ? 24 : 8),
                  child: _EventCard(
                    event: event,
                    onEdit: () => _editEvent(event),
                    onDelete: () => _deleteEvent(event),
